...
export function createJobsRouter(coachingService: CoachingService, whatsAppClient: WhatsAppClient, jobTriggerToken: string): Router {
  const router = Router();
  ...
  router.post("/jobs/daily-reminders", async (_req, res) => {
    try {
      ...
      await sendAllBatches(whatsAppClient, batches);
...
// Gleiches Versand-Prinzip wie routes/whatsapp.webhook.ts: CoachingService
// kennt keinen WhatsAppClient direkt, liefert nur die Nachrichten zurueck --
// der Route-Handler sendet sie ueber die passende Methode.
async function sendAllBatches(
  whatsAppClient: WhatsAppClient,
  batches: Array<{ whatsappNumberHash: string; messages: import("../types/domain.js").OutgoingMessage[] }>
): Promise<void> {
  for (const batch of batches) {
    for (const message of batch.messages) {
      if (message.type === "text") await whatsAppClient.sendText(batch.whatsappNumberHash, message.text);
      else if (message.type === "video") await whatsAppClient.sendVideo(batch.whatsappNumberHash, message.videoUrl, message.caption);
      else if (message.type === "image") await whatsAppClient.sendImage(batch.whatsappNumberHash, message.imageUrl, message.caption);
      else if (message.type === "quickReply") await whatsAppClient.sendQuickReply(batch.whatsappNumberHash, message.text, message.options);
    }
  }
}
